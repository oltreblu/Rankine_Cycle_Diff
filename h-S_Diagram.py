...
if __name__ == '__main__':
    ...
    efficiencies = np.linspace(0,1,100)

    results = []
    war_index = 200
    for eff in efficiencies:
        params = [(p_i, p_f, h_i, s_i, eff, m_t) for m_t in range(1, turbines + 1)]
        ...
        results.append(round((max_result - min_result) / 4186, 2))
        print(f'There are still {war_index} iterations to do.')
        war_index -=1


    pl.figure(1)
    pl.plot(efficiencies, results)
    pl.title('How the increase in resulted enthalpy is based on the expansion efficiency of turbines')
    pl.xlabel('Efficiency')
    pl.ylabel('Δh_increase')
    pl.show()

    print(f'It took {time.time() - starting_time} seconds')from CoolProp.CoolProp import PropsSI
import numpy as np
import time
from multiprocessing import Pool
import matplotlib.pyplot as pl
